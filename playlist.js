...
function reloadPlaylist(sCallback) {
  ...
  if (
    oCurrentPlaylist &&
    oCurrentPlaylist.startReload &&
    oCurrentPlaylist.type === "url" &&
    window.Worker &&
    !bDownloadRunning &&
    oCurrentPlaylist.grabtime < iAppStartTime - 120000
  ) {
...
function setActivePlaylist(sId, sOnSuccess, sOnFailure) {
  ...
  if (
    aLoadedPlaylists &&
    aLoadedPlaylists[sId] &&
    aLoadedPlaylists[sId].channelCount
  ) {
    oChannelEpgIds = {};
    ...
    sPlaylistNav =
      '<li id="current_playlist" data-prev="main_nav" data-next="main_nav">' +
      oCurrentPlaylist.name +
      " (" +
      oCurrentPlaylist.channelCount +
      ")" +
      "</li>";
  ...
  if (bDbInitiated && oDb) {
    ...
    oRequest.onsuccess = function (event) {
      ...
      if (oRecord && oRecord.value) {
        ...
        var sHtml =
          '<button class="button icon-button edit focusable" onclick="editPlaylist(' +
          aPlaylist.id +
          ')"></button><p>' +
          sName;
        if (aPlaylist.url) {
    ...
    oTx.oncomplete = function () {
      ...
      for (var i in aLoadedPlaylists) {
        ...
        sHtml +=
          '<li data-pid="' + i + '" ' + sSelected + ">" + sName + "</li>";
      }

      sHtml +=
        '<li id="selector_open_playlist_manager" class="i18n icon icon-settings" data-langid="playlistSelectorOpenPm">' +
        getLang("playlistSelectorOpenPm") +
        "</li>";
...
function setCategoriesStatus() {
  getEl("category_live").classList.toggle(
    "visible",
    oCurrentPlaylist.liveCount > 0
  );
  getEl("category_movie").classList.toggle(
    "visible",
    oCurrentPlaylist.movieCount > 0
  );
  getEl("category_series").classList.toggle(
    "visible",
    oCurrentPlaylist.seriesCount > 0
  );
}
...
function pickPlaylistSelector(oNavItem, bSkipPremiumCheck) {
  ...
  setActivePlaylist(
    ...
    function () {
      ...
      if (
        oCurrentPlaylist.lastChannel &&
        AppSettings.isActive("startup-last-channel")
      ) {
...
function showXstreamSeriesSelector(oJson) {
  ...
  for (var i in aSeriesEpisodes) {
    ...
    for (var x in oSeason) {
      ...
      sHtml +=
        '<li data-sid="' + i + '" data-eid="' + x + '">' + sName + "</li>";
    }
  }

  oSeriesNavSelector.innerHTML = sHtml;
  getEl("series_headline").innerHTML =
    "(EXPERIMENTAL function!)<br><h2>" + aInfo.name + "</h2>";
