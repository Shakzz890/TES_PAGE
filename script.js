// This is your new, complete script.js file.
...
// ... (Your EPG_URLS and i18n objects remain unchanged)
...
/* --------------------- Playback (JW PLAYER) & UI --------------------- */

// 🛑 REMOVED: `setupHlsForUrl` function is no longer needed.
...
function toggleFavourite() {
    ...
    if (newIndex !== -1) {
        ...
        playerInstance.stop(); // Use JW Player API
        showIdleAnimation();
    }
...
function onFirstUserAction() {
    ...
    playerInstance.setMute(false); // Use JW Player API
    if (oIdleAnimation) oIdleAnimation.classList.add('HIDDEN');
}

/* ✅ REWRITTEN: load channel using JW Player API */
function loadChannel(index) {
    ...
    // 1. Create a playlist item for JW Player
    const playlistItem = {
    ...
    // 2. Add DRM configuration for ClearKey channels
    ...
    // 3. Load the content into the player
    try {
        playerInstance.load([playlistItem]);
        playerInstance.play(); // Tell player to play the new source
        hideIdleAnimation(); // Hide overlay now that we are playing
    } catch (error) {
        console.error('Error loading channel with JW Player', channel.name, error);
        showIdleAnimation();
    } finally {
        // JW Player events handle loading state better
        // We can listen to `on('play')` to hide the loader
    }
}
...
function buildNav() {
    ...
    setTimeout(() => {
        ...
        // ✅ MODIFIED: Autoplay logic for JW Player
        if (isInitialLoad && aFilteredChannelKeys.length > 0) {
            ...
            // Autoplay is muted by default in player setup
            // The `loadChannel` function now also calls `hideIdleAnimation`
...
// ... (The rest of your UI functions like renderChannelSettings, updateStreamInfo, showChannelName, etc., remain unchanged)
function renderChannelSettings(){ oChannelSettingsList.innerHTML=''; if(aFilteredChannelKeys.length===0) return; const currentKey = aFilteredChannelKeys[iCurrentChannel]; const channel = channels[currentKey]; channelSettingsOptions.forEach((opt, index) => { const item = document.createElement('li'); item.id = opt.id; let text = getLang(opt.langid); if(opt.id === 'channel-setting-favourite'){ text = channel && channel.favorite ? 'Remove from Favourites' : 'Add to Favorites'; } item.textContent = text; if(index === iChannelSettingsIndex) item.classList.add('selected'); oChannelSettingsList.appendChild(item); }); }
function updateStreamInfo(){ if(!oStreamInfo) return; oStreamInfo.innerHTML = 'Stream: ' + (aFilteredChannelKeys[iCurrentChannel] ? channels[aFilteredChannelKeys[iCurrentChannel]].name : 'N/A'); }
function showChannelName(){ clearTimeout(channelNameTimeout); if(aFilteredChannelKeys.length===0) return; const key = aFilteredChannelKeys[iCurrentChannel]; if(!key) return; const channel = channels[key]; if(!channel) return; getEl('channel_name').textContent = channel.name; const epgProgs = findEpgForChannel(channel); let epgText = 'EPG not available'; if (epgProgs) { const { now, next } = getNowAndNextFromProgList(epgProgs); if(next) { const t = next.start ? next.start : null; if(t){ const local = new Date(t.getTime()); const hh = String(local.getHours()).padStart(2,'0'); const mm = String(local.getMinutes()).padStart(2,'0'); epgText = `Next: ${next.title}${next.sub ? ' - '+next.sub : ''} (${hh}:${mm})`; } else { epgText = `Next: ${next.title||'Unknown'}`; } } else { epgText = 'EPG not available'; } } getEl('channel_epg').innerHTML = epgText; getEl('ch_logo').innerHTML = channel.logo?`<img src="${channel.logo}" alt="">`:''; oChannelInfo.classList.add('visible'); channelNameTimeout = setTimeout(hideChannelName, 5000); }
...
/* ✅ MODIFIED: Keyboard controls for JW Player */
    ...
    if (bNavOpened) {
        ...
        switch(e.key){
            ...
            case ' ': // Space - toggle play/pause
                if (playerInstance.getState() === 'playing') playerInstance.pause();
...
// ... (Your Search and Touch handlers remain unchanged)
oSearchField && oSearchField.addEventListener('input', () => { iCurrentChannel = 0; buildNav(); });
...
function handleTouchEnd(e){ if(touchStartX===0) return; const deltaX = touchEndX - touchStartX, deltaY = touchEndY - touchStartY; const absDeltaX = Math.abs(deltaX), absDeltaY = Math.abs(deltaY); const swipeThreshold = 50, tapThreshold = 10; const target = e.target; const isInsideScrollable = oNav.contains(target) && target.closest('.custom-scrollbar'); if (isInsideScrollable) { if (absDeltaX>absDeltaY && absDeltaX>swipeThreshold) { if (deltaX>0) { /* swipe right inside list */ } else { /* swipe left inside list */ } } } else if (absDeltaX < tapThreshold && absDeltaY < tapThreshold) { if (bGuideOpened && !oGuide.querySelector('.fullscreen-popup').contains(target)) { hideGuide(); } else if (bNavOpened && !oNav.contains(target)) { hideNav(); } else if (bChannelSettingsOpened && !oChannelSettings.contains(target)) { hideChannelSettings(); } else if (!bNavOpened && !bChannelSettingsOpened && !bGuideOpened && !target.closest('#play_button_overlay')) { showChannelName(); } } else if (absDeltaX > swipeThreshold || absDeltaY > swipeThreshold) { if (absDeltaX > absDeltaY) { if (deltaX > 0) { if (bChannelSettingsOpened) hideChannelSettings(); else if (bNavOpened && !bGroupsOpened) showGroups(); else showNav(); } else { if (bNavOpened && bGroupsOpened) hideGroups(); else if (bNavOpened) hideNav(); else showChannelSettings(); } } else { if (!bNavOpened && !bChannelSettingsOpened && !bGuideOpened) { if (deltaY > 0) { loadChannel(iCurrentChannel-1) } else { loadChannel(iCurrentChannel+1) } } } } touchStartX=touchStartY=touchEndX=touchEndY=0; }
...
// 🛑 REMOVED: Custom playback controls setup, as JW Player has its own UI for this.

// ... (updateSelectedChannelInNav remains unchanged)
function updateSelectedChannelInNav() { if (!oChannelList) return; const currentSelected = oChannelList.querySelector('.selected'); if (currentSelected) currentSelected.classList.remove('selected'); const listItems = oChannelList.querySelectorAll('li.channel-item'); const newSelected = listItems[iCurrentChannel]; if (newSelected) { newSelected.classList.add('selected'); if (bNavOpened) { newSelected.scrollIntoView({ behavior: 'smooth', block: 'center' }); } } }
...
(function initApp(){
    ...
    playerInstance = jwplayer("player").setup({
        ...
        mute: true,       // Mute on start to allow autoplay
        displaytitle: false,
        controls: true // Show JW Player controls on hover
    });
    
    // ✅ Add event listeners to sync UI with player state
    ...
    // --- Continue with original app setup ---
    let i = 1;
