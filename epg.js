...
function parseEpgTime(sTimeString, iOffset) {
  ...
  var offsetMinutes = parseInt(
    iOffset.substring(0, 1) + iOffset.substring(3, 5),
    10
  ); // Keeps sign
  ...
  return new Date(
    Date.UTC(year, month, day, hour, minute, second) -
      (offsetHours * 60 + offsetMinutes) * 60000
  );
}
...
function isEpgOutdated(sLastUpdateTime) {
  if (sLastUpdateTime) {
    ...
    var bIsOutdated =
      Date.now() - iGrabIntervalSetting * 3600000 > parseInt(sLastUpdateTime);
    if (!bIsOutdated) {
...
function setSmartEpg() {
  var sChannelNumberLogo =
    '<span class="list-ch">' +
    displayChannelNumber(iCurrentChannel) +
    "</span>";
  if (sCurrentChannelLogo) {
    sChannelNumberLogo =
      '<img src="' + sCurrentChannelLogo + '">' + sChannelNumberLogo;
  }
...
function createChannelEpgProgram(iPos) {
  ...
  if (oProgram) {
    ...
    if (oProgram.startTime) {
      sProgrammHtml +=
        '<p class="p-time"><b class="HR"></b>' +
        getTimeString(oProgram.start, { showDate: true }) +
        ", " +
        oProgram.startTime +
        " – " +
        oProgram.endTime +
    ...
    sProgrammHtml +=
      '<div class="p-content custom-scrollbar" style="max-height: 100%;">';
    ...
    if (oProgram["episode-num"] || oProgram.date) {
      ...
      if (oProgram["episode-num"]) {
        sProgrammHtml +=
          '<span class="epg-program-episode">' +
          oProgram["episode-num"] +
          "</span>";
        if (oProgram.date) {
          sProgrammHtml += ", ";
        }
      ...
      if (oProgram.date) {
        if (/^\d{8}$/.test(oProgram.date)) {
          ...
          sProgrammHtml +=
            '<span class="epg-program-date">' +
            oDate.toLocaleDateString() +
            "</span>";
        } else {
          sProgrammHtml +=
            '<span class="epg-program-date">' + oProgram.date + "</span>";
        }
      }
    ...
    if (oProgram.desc) {
      sProgrammHtml +=
        '<p class="p-desc">' + oProgram.desc.replace(/\n/g, "<br>") + "</p>";
    }
    ...
    if (oProgram.hasCatchup) {
      sTags +=
        '<div class="epg-tag p-arch" onclick="smartControlAction(13);"><span>Catchup</span></div>';
    }
...
function updateChannelNameEpg(oProgram) {
  ...
  if (!bJumpNext && iElapsedPct >= 100) {
    ...
    oChannelEpg.innerHTML =
      ...
      '</div><div class="stop">' +
      oProgram.endTime +
      '</div><div id="channel_info_epg_timeline"><div id="channel_info_epg_elapsed" style="width: ' +
      iElapsedPct +
      '%"></div></div><div id="channel_info_epg_title">' +
      oProgram.title +
      "</div>";
    bJumpNext = false;
  }
...
function setEpgLoadStatus(iStatus) {
  ...
  switch (iStatus) {
    ...
    case 0:
      ...
      oChannelEpg.innerHTML =
        '<span class="e-loading">' + getLang("loading") + "</span>";
      break;
...
function loadChannelEpgIds(sEpgId, sCallback, sErrorCallback) {
  ...
  var singleKeyRange = IDBKeyRange.only(sEpgId),
    oRequest = oDb
      .transaction("epgChannels")
      .objectStore("epgChannels")
      .index("ch_name");
    ...
    if (!sEpgId) {
      ...
      oRequest.onsuccess = function (event) {
        ...
        if (iMaxProgramCount) {
          ...
          oEpgChannelContent.style.transform =
            "translateX(" + iCurrentProgramPos * -100 + "%)";
...
function buildEpgNavList() {
  ...
  if (aFilteredChannelList.length === 0) {
    ...
    oEpgChannelList.innerHTML =
      '<ul id="epg_nav_channels" style="height: ' + iHeight + 'px"></ul>';
  ...
  getChannelEpgId(aActiveChannelList[iChNum], function (sEpgId) {
    ...
    if (!oInsert) {
      if (typeof aFilteredChannelList[iChNum] !== "undefined") {
        ...
        oInsert.style.top =
          aFilteredChannelList[iChNum] * iNavChannelHeight + "px";
      }
    }
      ...
      for (var i = 0; i < iProgramsCount; i++) {
        ...
        if (getProgramDuration(oStartTime, oEndTime)) {
          if (oStartTime.getTime() < iDateNow) {
            var iElapsedPct = Math.round(
              ((iDateNow - oStartTime.getTime()) /
                (oEndTime.getTime() - oStartTime.getTime())) *
                100
            );
            ...
            sActive = " p-live";
            sTimelineWidth =
              'style="background-image: linear-gradient(90deg, var(--theme-dark) 0, var(--theme-moderate) ' +
              iElapsedPct +
              "%, var(--theme-dark) " +
              iElapsedPct +
              '%)"';
        ...
        oEpgProgramContainer.innerHTML +=
          '<div class="p-i' +
          sActive +
          '" ' +
          sTimelineWidth +
