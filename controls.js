...
function smartControlMoveEpgProgram(bSmooth) {
  ...
  getEl("smart_controls").classList.toggle(
    "arch-play-icon",
    oEpgProg && oEpgProg.classList.contains("p-arch")
  );
}
...
function smartControlAction(k) {
  switch (k) {
    case aButtons["KeyUp"]:
      ...
      if (
        sSmartControlActive === "player" ||
        sSmartControlActive === "playback"
      ) {
...
function updateProgressBar(iCurTime) {
  ...
  getEl("time_progress").innerText =
    transformTime(iCurTime, bAddHours) + " / " + transformTime(iDur, bAddHours);
}
...
function initMouseControls() {
  ...
  document
    .getElementById("epg_nav_list_container")
  ...
  oPlaybackController.addEventListener(
    "mouseout",
    function (ev) {
      clearTimeout(iHideArrowTimeout);
      iHideArrowTimeout = setTimeout(function () {
...
function initControls() {
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (
      (sSmartControlActive || bProgramBrowserOpened) &&
      smartControlAction(k)
    ) {
      e.preventDefault();
      return;
    }
    ...
    if (bChannelSettingsOpened) {
      if (sChannelSetting) {
        ...
        if (sChannelSetting === "sub-dub") {
          ...
          oSettingSelectBoxes = document.querySelectorAll(
            "#channel_settings_video .setting-button"
          );
        }
    ...
    if (bNavOpened) {
      ...
      if (bChannelEditModeActive) {
        ...
        switch (k) {
          case 37: // LEFT
          case 39: // RIGHT
            toggleChannelEditMode(
              sChannelEditMode == "move" ? "delete" : "move"
            );
            break;
